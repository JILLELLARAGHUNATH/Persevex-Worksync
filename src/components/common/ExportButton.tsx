...
export default function ExportButton({ action, fileName = 'Export_Report', label = 'Export Excel (.xlsx)' }: ExportButtonProps) {
  ...
  return (
    ...
    >
      {loading ? <Loader2 className="w-4 h-4 animate-spin text-indigo-600 dark:text-indigo-400" /> : <FileSpreadsheet className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />}
