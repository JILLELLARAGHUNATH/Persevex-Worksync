...
export default function SalaryPayrollClient({
  ...
  return (
    <div className="space-y-3">
      {/* Compact Payroll Header */}
      <div className="ws-card px-4 py-3 border-l-4 border-l-indigo-500 rounded-xl">
        {/* Row 1: Title + month nav + actions */}
        <div className="flex flex-wrap items-center justify-between gap-2 mb-2.5">
          <div className="flex items-center gap-3">
            <div>
              <div className="flex items-center gap-2">
                <h1 className="text-sm font-bold text-slate-900 dark:text-slate-100">Salary &amp; Payroll</h1>
                {loading && <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-500" />}
                {summary.status === 'FINALIZED' && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-950/60 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 flex items-center gap-1">
                    <ShieldCheck className="w-3 h-3" /> Finalized
                  </span>
                )}
                {summary.status === 'RECALCULATION_REQUIRED' && (
                  <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800/60 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" /> Recalculate
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-500 dark:text-slate-400">{applicableWorkingDays} working days &middot; {MONTH_NAMES[month - 1]} {year}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700">
              <button onClick={handlePrevMonth} disabled={loading || actionLoading} className="p-1.5 rounded-l-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-white dark:hover:bg-slate-900 transition cursor-pointer disabled:opacity-50" title="Previous Month"><ChevronLeft className="w-3.5 h-3.5" /></button>
              <span className="px-2 text-xs font-semibold text-slate-700 dark:text-slate-300 font-mono select-none">{MONTH_NAMES[month - 1]} {year}</span>
              <button onClick={handleNextMonth} disabled={loading || actionLoading} className="p-1.5 rounded-r-lg text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100 hover:bg-white dark:hover:bg-slate-900 transition cursor-pointer disabled:opacity-50" title="Next Month"><ChevronRight className="w-3.5 h-3.5" /></button>
            </div>
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-lg transition border border-slate-200 dark:border-slate-700 flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
            ...
            <div className="relative">
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold rounded-lg transition border border-slate-200 dark:border-slate-700 flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                {exportLoading ? (<Loader2 className="w-3.5 h-3.5 animate-spin text-emerald-500" />) : (<Download className="w-3.5 h-3.5 text-slate-600 dark:text-slate-400" />)}
                <span>Export</span>
              ...
              {exportMenuOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-1.5 w-52 ws-card shadow-xl z-50 py-1.5 animate-in fade-in-50 zoom-in-95 duration-100">
                    <div className="px-3 py-1.5 border-b border-slate-100 dark:border-slate-800">
                      <p className="text-[10px] uppercase font-bold tracking-wider text-slate-400">Export {MONTH_NAMES[month - 1]} {year}</p>
                    </div>
                    <button onClick={() => handleExport('csv')} className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 transition cursor-pointer">
                      <FileText className="w-4 h-4 text-emerald-600 dark:text-emerald-400" />
                      <div><p className="font-semibold text-slate-800 dark:text-slate-200">Download CSV</p><p className="text-[10px] text-slate-400">Comma-separated values (.csv)</p></div>
                    </button>
                    <button onClick={() => handleExport('xlsx')} className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 transition cursor-pointer">
                      <FileSpreadsheet className="w-4 h-4 text-blue-600 dark:text-blue-400" />
                      <div><p className="font-semibold text-slate-800 dark:text-slate-200">Download Excel</p><p className="text-[10px] text-slate-400">Microsoft Excel format (.xlsx)</p></div>
                    </button>
                    <button onClick={handlePrintPDF} className="w-full px-3 py-2 text-left text-xs font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 flex items-center gap-2 transition cursor-pointer border-t border-slate-100 dark:border-slate-800">
                      <Printer className="w-4 h-4 text-purple-600 dark:text-purple-400" />
                      <div><p className="font-semibold text-slate-800 dark:text-slate-200">Print / Save as PDF</p><p className="text-[10px] text-slate-400">Formatted summary printout</p></div>
                    </button>
                  </div>
                </>
              )}
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-gradient-to-r from-emerald-600 to-emerald-500 hover:from-emerald-500 hover:to-emerald-400 text-white text-xs font-semibold rounded-lg transition shadow-sm disabled:opacity-50 flex items-center gap-1.5 cursor-pointer"
            >
        ...
        <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Net Payable</div>
            <div className="text-base font-black text-slate-900 dark:text-white font-mono">&#8377;{summary.totalPayableSalary.toLocaleString('en-IN')}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Workforce</div>
            <div className="text-base font-black text-slate-900 dark:text-white font-mono">{summary.totalEmployees}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Gross Base</div>
            <div className="text-base font-black text-slate-900 dark:text-white font-mono">&#8377;{summary.totalMonthlySalary.toLocaleString('en-IN')}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Paid Leaves</div>
            <div className="text-base font-black text-emerald-700 dark:text-emerald-400 font-mono">{summary.totalPaidLeaveDays}</div>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5">
            <div className="text-[10px] text-slate-500 font-semibold uppercase tracking-wider">Unpaid Days</div>
            <div className="text-base font-black text-amber-700 dark:text-amber-400 font-mono">{summary.totalUnpaidDays}</div>
          </div>
        </div>
      </div>
      ...
      {/* Filter Toolbar */}
      <div className="ws-card p-3 flex flex-wrap items-center justify-between gap-2.5 text-xs">
      ...
      <div className="ws-card p-4 text-xs space-y-2 text-slate-600 dark:text-slate-400 shadow-xs transition-colors">
      ...
      {detailModalUser && (
        <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="ws-card max-w-3xl w-full p-5 shadow-2xl animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto space-y-4">
